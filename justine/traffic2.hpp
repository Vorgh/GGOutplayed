...
public:
    ...
    :m_size ( size ), m_catchdist ( catchdist ), m_type ( type ), m_minutes ( minutes )
  {
    ...
    rendorfile = "RENDORFILE " + boost::posix_time::to_simple_string ( now );
    RENDORFILE = new std::fstream ( rendorfile.c_str() , std::ios_base::out );
                ...
                if ( d < m_catchdist )
                  {
                    ...
                    *RENDORFILE << "Elkapas!  Rendor: " << *car1 << " | Ido: " << m_time << " | Pozicio: " << lon1 << "," << lat1 << std::endl;
...
private:
  ...
  std::fstream* logFile;
  std::string logfile;
  std::fstream* RENDORFILE;
  std::string rendorfile;
  //std::ofstream RENDORFILE("RENDORFILE " + boost::posix_time::to_simple_string ( boost::posix_time::second_clock::universal_time() ), std::ios_base::out | std::ios_base::app);
