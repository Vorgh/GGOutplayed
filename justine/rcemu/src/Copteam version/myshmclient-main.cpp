...
int main ( int argc, char* argv[] )
{
     ...
     ( "cops", boost::program_options::value< std::string > (), "number of cops" )
     ( "teamsize", boost::program_options::value< std::string > (), "number of cops in a team" )
     ;
     ...
     std::string cops;
     if ( vm.count ("cops"))
       cops.assign (vm["cops"].as<std::string> () );
     else
       cops.assign ("10");
     
     std::string teamsize;
     if ( vm.count ("teamsize"))
       teamsize.assign (vm["teamsize"].as<std::string> () );
     else
       teamsize.assign ("3");
     ...
     */
     ...
     try {
          ...
          myShmClient.start ( io_service, port.c_str(), std::stoi(cops), std::stoi(teamsize) );
