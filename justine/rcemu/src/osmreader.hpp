...
public:
  ...
  void way ( osmium::Way& way )
  {
    ...
    if (maxspeed)
    {
      std::string s(maxspeed);
      if (!strcmp(maxspeed, "maxspeed"))
      {
	    if (isdigit(maxspeed[2]))
	    {
	      speedlimit = atoi(s.substr(0,3).c_str());
	    }
	    else
	    {
	      if (isdigit(maxspeed[1]))
		speedlimit = atoi(s.substr(0,2).c_str());
	    }
      }
    }*/
            ...
            if ( !edge ( vertex_old, vertex ) )
              {
                ...
                palist[vertex_old].push_back ( edge_length / speedmod );
                ...
                if ( !edge ( vertex, vertex_old ) )
                  {
                    ...
                    palist[vertex].push_back ( edge_length / speedmod );
