...
public:
  ...
  void start ( boost::asio::io_service& io_service, const char * port, int number );
  
  //int DistanceDifference(std::map<int, double[5]> distances, unsigned int g, unsigned int t);
...
private:
  ...
  std::vector<Cop> initcops ( boost::asio::ip::tcp::socket & socket, int number);
